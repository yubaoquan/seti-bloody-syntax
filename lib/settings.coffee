...
module.exports =
  init: (state) ->
    ...
    if self.isLoaded('seti-bloody-syntax')
      ...
      if self.isLoaded('seti-bloody-ui')

        # IF DYNAMIC THEM IS ALLOWED
        if atom.config.get('seti-bloody-syntax.dynamicColor') and not @themeSet
          # SET SYNTAX THEME TO MATCH UI
          self.setTheme atom.config.get('seti-bloody-ui.themeColor'), false, false
        ...
        self.onDeactivate 'seti-bloody-ui', ->
      ...
      if (atom.config.get('seti-bloody-syntax.themeColor')) and not @themeSet
        self.setTheme atom.config.get('seti-bloody-syntax.themeColor'), false, false
